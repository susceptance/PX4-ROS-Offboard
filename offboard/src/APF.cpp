...
int main(int argc, char** argv)
{
    ...
    while(ros::ok())
    {
		...
		//if(current_state.armed && current_pose.pose.position.z >= 2 && !Control_pose)  
    	{
            GPS_mission = true;
			pz = 2;
       	...
       	if(GPS_mission)
		{
       	...
       	if(cam_scan)
       	{
			...
			//for(int i = 0; i < 5; i++)
			//	std::cout << i << ". -- before tmp_obs : " << tmp_obs_x[i] << "  " << tmp_obs_y[i] << "  " << tmp_obs_z[i] << std::endl;
			...
			//for(int i = 0; i < 5; i++)
			//	std::cout << i << ". -- after tmp_obs : " << tmp_obs_x[i] << "  " << tmp_obs_y[i] << "  " << tmp_obs_z[i] << std::endl;
		...
		if(object_detect)
		{
			...
			/*for(int i = 0; i < 10; i++)
			{
				std::cout << i << ". -- cam_obs : " << tmp_obs_x[i] << "  " << tmp_obs_y[i] << std::endl;
			}*/
			...
			for(int obs = 0; obs < 16; obs++)
			{
				...
				//std::cout << "cam : " << obs_cam[0][0] << " " << obs_cam[1][0] << " " << obs_cam[2][0] << std::endl;
			...
			//for(int i = 0; i < 10; i++)
				//std::cout << i << ". -- world obs : " << obs_x[i] << "  " << obs_y[i] << "  " << obs_z[i] << std::endl;
